...
const BTN_ADD_USER = <HTMLInputElement>document.getElementById('btn-add-edit');
...
function saveUser(index: number, login: string, password: string, email: string) {
...
function editUser(index) {
    (<HTMLFormElement>FORM).elements['login'].value = data[index].login;
    (<HTMLFormElement>FORM).elements['password'].value = data[index].password;
    (<HTMLFormElement>FORM).elements['email'].value = data[index].email;
}
...
function addUser() {
    let obj = {
        login: (<HTMLFormElement>FORM).elements['login'].value,
        password: (<HTMLFormElement>FORM).elements['password'].value,
        email: (<HTMLFormElement>FORM).elements['email'].value
    }
...
FORM.addEventListener('submit', function (event) {
    ...
    if (editUserIndex >= 0) {
        saveUser(editUserIndex, (<HTMLInputElement>event.target['login']).value, (<HTMLInputElement>event.target['password']).value, (<HTMLInputElement>event.target['email']).value);
